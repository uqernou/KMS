...
if os.path.exists(r"C:\Users\uqern\Desktop\KMS\Dane2\characteristic_1.txt"):
    with open(r"C:\Users\uqern\Desktop\KMS\Dane2\characteristic_1.txt") as fobj:
        for j, line in enumerate(fobj):
            row = line.split()
            x2.append(row[0])
            rho2.append(row[1])
if os.path.exists(r"C:\Users\uqern\Desktop\KMS\Dane2\data_4999.txt"):
    with open(r"C:\Users\uqern\Desktop\KMS\Dane2\data_4999.txt") as fobj:
        for line in fobj:
            row = line.split()
            time2.append(row[0])
            norma2.append(row[1])
...
def update(i):
    ...
    time = [time2[a] for a in range(i+3990)]
    norma = [norma2[a] for a in range(i+3990)]
    polozenie_sr = [polozenie_sr2[a] for a in range(i+3990)]
    energia = [energia2[a] for a in range(i+3990)]
